...
resultat_nom = process.extract(choix_joueur, df_recherche['name'].to_list(), score_cutoff = 50, limit = 5)
...
if choix_joueur:    
    ...
    if len(df_recherche) == 0:
        ...
        for n in range(len(resultat_nom)):
            ...
            id = df_recherche[df_recherche['name'] == resultat_nom[n][0]]['ID'].iloc[0]
        ...
        for m in range(len(liste_id_resultat_nom)):
            ...
            nom_annee = df_recherche[df_recherche['ID'] == liste_id_resultat_nom[m]]['Nom_annee'].iloc[0]
            liste_nom_annee.append(nom_annee)

        # liste_nom = []
        # for n in range(len(resultat_nom)):
        #     liste_nom.append(resultat_nom[n])
        name = st.selectbox("Quel joueur parmi notre base précisément ?",(liste_nom_annee))
        ...
        if resultats:
            ...
            df_recherche['name'] = df_recherche['name'].apply(lambda x : x.lower())
            df_recherche['ID'] = df_recherche['ID'].astype(str)
            df_recherche = df_recherche[df_recherche['Nom_annee'] == name]
